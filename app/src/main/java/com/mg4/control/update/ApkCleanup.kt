...
object ApkCleanup {
    ...
    /**
     * Vrai si [name] est un APK de MG4Control dans les Téléchargements.
     * Couvre le naming actuel `MG4Control-<flavor>-<version>.apk` ET l'ancien `MGControl<version>.apk`
     * (pour nettoyer aussi les téléchargements déjà présents avant le changement de nom).
     */
    fun isAppApk(name: String): Boolean =
        (name.startsWith("MG4Control", ignoreCase = true) ||
